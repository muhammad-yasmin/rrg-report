<?php 
	...
	$totalSales = mysqli_query($connect,"SELECT COUNT(namaItem) AS total FROM tbl_em WHERE type='Sale' AND tanggal <= '".$realTo."' AND tanggal >= '".$realFrom."'");
	$country = mysqli_query($connect,"SELECT negara, count(*) AS total FROM tbl_em WHERE NOT negara = '' AND tanggal <= '".$realTo."' AND tanggal >= '".$realFrom."' GROUP BY negara ORDER BY Total DESC LIMIT 1");
	$bestSales = mysqli_query($connect,"SELECT namaItem, count(*) AS total FROM tbl_em WHERE type='Sale' AND tanggal <= '".$realTo."' AND tanggal >= '".$realFrom."' GROUP BY namaItem ORDER BY Total DESC LIMIT 1");
	$saleReversal = mysqli_query($connect,"SELECT COUNT(namaItem) AS total FROM tbl_em WHERE type='Sale Reversal' AND tanggal <= '".$realTo."' AND tanggal >= '".$realFrom."'");
	$bestTwo = mysqli_query($connect, "SELECT namaItem, COUNT(namaItem) AS total FROM tbl_em WHERE type='Sale' AND tanggal <= '".$realTo."' AND tanggal >= '".$twoWeek."' GROUP BY namaItem ORDER BY Total DESC LIMIT 1");
?>
