...
<table class="table table-hover dataTable w-full" data-plugin="dataTable" id="tabelnya">
	...
	<tbody>
		...
		$tahun = date("Y");
		$bulan = date("m");
		$sql = "SELECT itemID, namaItem, count(namaItem) AS totalSale FROM tbl_em WHERE type='Sale' AND tanggal <= '".$tahun."-".$bulan."-31' AND tanggal >= '".$tahun."-".$bulan."-01' GROUP BY namaItem";
