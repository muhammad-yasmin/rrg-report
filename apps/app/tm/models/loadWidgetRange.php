<?php 
	...
	$totalSales = mysqli_query($connect,"SELECT COUNT(namaItem) AS total FROM tbl_tm WHERE tanggal <= '".$realDate."' AND tanggal >= '".$dateBack."'");
	//$country = mysqli_query($connect,"SELECT negara, count(*) AS total FROM tbl_tm WHERE NOT negara = '' AND tanggal <= '".$realDate."' AND tanggal >= '".$dateBack."' GROUP BY negara ORDER BY Total DESC LIMIT 1");
	$bestSales = mysqli_query($connect,"SELECT namaItem, count(*) AS total FROM tbl_tm WHERE tanggal <= '".$realDate."' AND tanggal >= '".$dateBack."' GROUP BY namaItem ORDER BY Total DESC LIMIT 1");
	//$saleReversal = mysqli_query($connect,"SELECT COUNT(namaItem) AS total FROM tbl_tm WHERE type='Sale Reversal' AND tanggal <= '".$realDate."' AND tanggal >= '".$dateBack."'");
	$bestTwo = mysqli_query($connect, "SELECT namaItem, COUNT(namaItem) AS total FROM tbl_tm WHERE tanggal <= '".$realDate."' AND tanggal >= '".$twoWeek."' GROUP BY namaItem ORDER BY Total DESC LIMIT 1");
?>
